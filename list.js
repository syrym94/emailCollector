const list = async (client, box, num) => {
    ...
    if (box) {
        try {
            for (let i = 0; i < box.length; i++) {
                let messages = await client.listMessages(`${box[i].name}`, `${box[i].emails}`, ['envelope', 'body.peek[]']) // Итерируем через массив отсеянных директории и вытаскиваем письма
                messages.forEach(async message => {
                    ...
                    try {
                        ...
                        if (comparedArrFrom == message.envelope.from[0].address || comparedArrTo == message.envelope.to[0].address) { // Если имеются совпадения то парсим тело письма и приклепленные файлы
                            ...
                            // console.log(responseToServer, 'responseToServer',num)
                            if (err) throw err;
                        } else {
